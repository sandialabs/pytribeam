...
class Progressbar(tk.Frame):
    ...
    def __init__(
        ...
        **extra,
        ...
        if size is not None:
            self.font = font or f"any {int((self.size // 2) * self.font_size_ratio)}"
        else:
            self.font = font or f"any {int(10 * self.font_size_ratio)}"
        ...
        self.percent_label = tk.Label(
            self, text="0%", bg=self.bg, fg=self.text_fg, font=self.font, anchor="w"
        )
        self.percent_label.grid(row=0, column=1, sticky="nsew")
...
class RadialProgressbar(tk.Frame):
    ...
    def create_style(self, fg="green", bg="white"):
        ...
        self.indicator_img = self.indicator_img or RadialProgressbar.imgs[
            self.size
        ].get(self.fg)
        ...
        try:
            s.element_create(
                indicator_element, "image", self.indicator_img, border=0, padding=0
            )
        except:
            pass
    ...
    def config(self, **kwargs):
        ...
        # bar style configure
        s.configure(
            self.bar_style, background=self.parent_bg, troughcolor=self.parent_bg
        )

        # percentage label
        self.percent_label.config(
            background=self.text_bg, foreground=self.text_fg, font=self.font
        )
...
class RadialProgressbar3d(RadialProgressbar):
    ...
    def __init__(
        self, parent, size=100, fg="cyan", text_bg="#333", text_fg="white", **extra
    ):
        """initialize progressbar
...
class Segmentbar(tk.Canvas):
    ...
    def update_bar(self, info):
        ...
        if tag_id:
            ...
            tag_id = self.create_rectangle(
                start, 0, end, self.height, fill=self.fg, width=0
            )
