...
def create_circle(
    size=100, thickness=None, color="black", fill=None, antialias=4, offset=0
):
...
def create_rectangle(size, color="black", fill=None, thickness=None):
    fill_color = color_to_rgba(fill) or "#0000"
    thickness = thickness or max(min(size) * 2 // 100, 2)
    img = Image.new(size=size, mode="RGBA", color="#0000")
    draw = ImageDraw.Draw(img)
    draw.rectangle(
        [0, 0, size[0], size[1]], outline=color, fill=fill_color, width=thickness
    )
    return img
...
def scroll_with_mousewheel(
    widget, target=None, modifier="Shift", apply_to_children=False
):
...
def theme_compatibility_check(print_warning=False):
    ...
    if current_theme not in compatible_themes:
        if print_warning:
            print(
                f"AwesomeTkinter Warning: Widgets might not work properly under current theme ({current_theme})\n"
...
def center_window(
    window, width=None, height=None, set_geometry_wh=True, reference=None
):
