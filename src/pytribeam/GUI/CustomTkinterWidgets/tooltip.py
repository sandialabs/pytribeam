...
class ToolTip:
    ...
    def showtip(self):
        ...
        self.kwargs["background"] = (
            self.kwargs.get("background") or self.kwargs.get("bg") or "#ffffe0"
        )
        self.kwargs["foreground"] = (
            self.kwargs.get("foreground") or self.kwargs.get("fg") or "black"
        )
...
def main():
    ...
    def foo():
        if x:
            l.tp.update_tooltip(
                f"counter: {x.pop()}"
            )  # or can use l.tp.text='some text'
            root.after(1000, foo)
...
def main():
    ...
    def foo():
        ...
        lbl.tp.update_tooltip(
            "Progress: " + str(lbl.counter) + "%"
        )  # or use l.tp.text='some text'
        root.after(1000, foo)
