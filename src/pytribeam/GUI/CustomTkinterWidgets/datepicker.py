...
class DatePicker(tk.Toplevel):
    ...
    def __init__(
        ...
        self.fields = {
            "Year": {"values": list(range(min_year, max_year + 1)), "selection": year},
    ...
    def create_widgets(self):
        ...
        tk.Label(top_frame, text="Select Date and time:", bg=bg, fg=fg).pack(
        ...
        s.configure(
            custom_style,
            arrowcolor=calc_font_color(arrow_bg),
            foreground=calc_font_color(textarea_bg),
            padding=4,
            relief=tk.RAISED,
        )
        s.map(
            custom_style,
            fieldbackground=[("", textarea_bg)],
            background=[("", arrow_bg)],
        )
        ...
        for key, item in self.fields.items():
            tk.Label(middle_frame, text=key, bg=bg, fg=fg).grid(
                row=0, column=c, padx=5, sticky="w"
            )
            cb = ttk.Combobox(
                middle_frame,
                state="readonly",
            ...
            cb.callback = lambda field_name=key, combo=cb: set_field(
                field_name, combo.get()
            )
        ...
        tk.Button(
            bottom_frame, text="Cancel", command=self.close, bg=btnbg, fg=btnfg
        ).pack(side="right", padx=5)
        tk.Button(
            bottom_frame, text="Ok", command=self.set_selection, bg=btnbg, fg=btnfg
        ).pack(side="right")
