...
def ebsd_average_ci(
    ...
    print("\tLogging average confidence index...")
    dataset_location = f"{step_number:02d}_{step_name}/{dataset_name}"
    time = current_time()

    with h5py.File(log_filepath, "r+") as file:
        if not dataset_location in file:
            avg_ci = file.create_dataset(
                dataset_location,
                (0,),
                Constants.ebsd_average_ci_dtype,
                maxshape=(None,),
            )
            avg_ci.attrs["Units"] = np.string_("[]")

        dataset = file[dataset_location]
        avg_ci_data = np.array(
            [
                (
                    slice_number,
                    round(avg_ci_p, 6),
        ...
        # add one row to table
        dataset.resize(dataset.shape[0] + 1, axis=0)
        dataset[-1:] = avg_ci_data


def ebsd_camera_saturation(
    step_number: int,
    step_name: str,
    slice_number: int,
    log_filepath: Path,
    ...
    print("\tLogging camera saturation...")
    dataset_location = f"{step_number:02d}_{step_name}/{dataset_name}"
    time = current_time()

    with h5py.File(log_filepath, "r+") as file:
        if not dataset_location in file:
            cam_sat = file.create_dataset(
                dataset_location,
                (0,),
                Constants.ebsd_camera_saturation_dtype,
                maxshape=(None,),
            )
            cam_sat.attrs["Units"] = np.string_("[]")

        dataset = file[dataset_location]
        cam_sat_data = np.array(
            [
                (
                    slice_number,
                    round(cam_sat_p, 6),
